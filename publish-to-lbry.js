...
(async function() {
  try {
    ...
    for(const episode of episodes.slice(-1)) {
      ...
      const name = `lesbertarian_${numberStr}`;
      console.log(name);

      const params = {
        name,
        bid: '1.0',
        file_path: path.join(videosDir, `lesbertarian_${numberStr}.mp4`),
        validate_file: true,
        optimize_file: true,
        title: episode.TITLE,
        description: episode.DESCRIPTION,
        thumbnail_url: 'https://lesbertarian.com/images/lesbertarian_video_16x9.png',
        author: 'MLGA Network',
        tags: [],
        languages: ['en'],
        locations: [{country: 'US'}],
        channel_name: '@Lesbertarian',
